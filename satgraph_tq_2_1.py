...
class UpdateBroadcastThread(threading.Thread):
    ...
    def update_broadcast_process(self):
        ...
        start_id = int((UpdatedVertex[-4]) * 100000 + (UpdatedVertex[-3]))
        end_id = int((UpdatedVertex[-2]) * 100000 + (UpdatedVertex[-1]))
...
class CalcThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            ### @@@@@@@
            UpdatedVertex[-5] = i
            #UpdatedVertex = np.append(UpdatedVertex, start_id)
            #UpdatedVertex = np.append(UpdatedVertex, end_id)
            UpdatedVertex[-4] = math.floor(start_id / 100000)
            UpdatedVertex[-3] = start_id % 100000
            UpdatedVertex[-2] = math.floor(end_id / 100000)
            UpdatedVertex[-1] = end_id % 100000
            UpdatedVertex = UpdatedVertex.astype(self.__Dtype_All['VertexData'])
...
class satgraph():
    ...
    def run(self, InitialVertex='zero'):
        ...
        UpdateVertexThread, TaskSchedulerThread, BroadVertexThread, UpdateBstThread, TaskThreadPool = self.create_threads()
        # if self.__MPIInfo['MPI_Rank'] == 0:
