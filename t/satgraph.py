...
def graph_to_matrix(Str_RawDataPath, Str_DestDataPath, Int_VertexNum, Int_PartitionNum, Dtype_All, Str_Seq=','):
  ...
  g_id  = 0                          # id
  g_p   = 10                         # total
  g_num = Int_PartitionNum/g_p       # number of partitions per id
  # vertex id from     g_id*g_num*Int_VertexPerPartition
  # vertex id to      (g_id+1)*g_num*Int_VertexPerPartition
  ######
  read_id = 0;
  read_id_stop = 0;
  ...
  for par_n in range(g_id*g_num, (1+g_id)*g_num):
    ...
    while True:
      ...
      if t_dst_array[-1] < (par_n)*Int_VertexPerPartition:
        read_id = read_id + 1
        continue
      if t_dst_array[-1] < (1+par_n)*Int_VertexPerPartition:
        ...
        read_id = read_id
      if new_partition_flag == True or read_id == read_id_stop:
        break
  ...
  _File_PartitionData = open(Str_DestDataPath + '/subdata/' + 'vertexout.' + str(g_id), 'w')
  _Array_VertexOut.tofile(_File_PartitionData)
  _File_PartitionData.close()
  _File_PartitionData = open(Str_DestDataPath + '/subdata/' + 'vertexin' + str(g_id), 'w')
  _Array_VertexIn.tofile(_File_PartitionData)
  _File_PartitionData.close()
  return Str_DestDataPath + '/subdata/', Int_NewVertexNum, Int_PartitionNum, Int_VertexPerPartition
...
if __name__ == '__main__':
  ...
  GraphInfo = graph_to_matrix('/data/4/eu-2015-t', './', 1070557254, 4000, Dtype_All);
