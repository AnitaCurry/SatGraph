...
class BroadThread(mp.Process):
    ...
    def update_BSP(self, updated_vertex, start_id, end_id):
        ...
        while 1:
            if self.__ControlInfo['IterationNum'].value == \
        ...
        while 1:
            ...
            UpdatedVertex, start_id, end_id = \
                self.__ControlInfo['CalcFunc'](i,
                                               self.__ControlInfo['IterationNum'].value,
                                               self.__DataInfo,
                                               self.__GraphInfo,
                                               self.__Dtype_All)
...
class SchedulerThread(mp.Process):
    __MPIInfo = {}
    __GraphInfo = {}
    __IP = '127.0.0.1'
    __TaskqPort = 17071
...
class satgraph():
    ...
    __IP = '127.0.0.1'
    __ThreadNum = 1
    __SM_IterationReport = None
    __SM_VertexVersion = None
    __SM_VertexOut = None
    __SM_VertexData = None
    __SM_VertexDataNew = None

    def __init__(self):
        self.__Dtype_All['VertexData'] = np.float32
        self.__Dtype_All['VertexEdgeInfo'] = np.int32
        self.__Dtype_All['EdgeData'] = np.bool
        self.__GraphInfo['DataPath'] = None
        self.__GraphInfo['VertexNum'] = None
        self.__GraphInfo['PartitionNum'] = None
        self.__ControlInfo['IterationNum'] = mp.Value(ctypes.c_int32, 0)
        self.__ControlInfo['IterationReport'] = None
        self.__ControlInfo['MaxIteration'] = 0
    ...
    def graph_process(self):
        ...
        NewIteration = False
        if self.__ControlInfo['IterationNum'].value != CurrentIterationNum:
            NewIteration = True
            if BSP:
                self.__DataInfo['VertexData'][:] = \
                    self.__DataInfo['VertexDataNew'][:]
            self.__ControlInfo['IterationNum'].value = CurrentIterationNum
