...
tmp_dfs_path = '/out_convert'
dst_path = "hdfs://bdp-10:9000/out_satgraph/"
...
_dst_flag = subprocess.call([Hadoop_bin, "fs", "-test", "-d", dst_path], stdout=subprocess.PIPE)
if _dst_flag != 0:
    sys.exit(0)

subprocess.call([Hadoop_bin, "fs", "-rmr", tmp_dfs_path], stdout=subprocess.PIPE)
subprocess.call([Hadoop_bin, "fs", "-rm", "/out_satgraph/*"], stdout=subprocess.PIPE)
subprocess.call(["rm "+tmp_path+"part-*"], shell=True, stdout=subprocess.PIPE)
...
def f1_1(i):
    ...
    subprocess.call([Hadoop_bin, "fs", "-get", "/out_convert/part-"+str(i).zfill(5), tmp_path+"part-"+str(i).zfill(5)], stdout=subprocess.PIPE)
    a = []
    b = []
    _file = open(tmp_path+"part-"+str(i).zfill(5), 'r')
    data = _file.read()
    ...
    for k in data:
        ...
        j += 1
    _file.close()
    subprocess.call(["rm "+tmp_path+"part-"+str(i).zfill(5)], shell=True, stdout=subprocess.PIPE)
    del data
    return a, b
...
for i in range(round_num):
    ...
    def f3(list_data):
        ...
        _file = open(tmp_path + str(partition_id) + '.edge', 'w')
        flushed_data.tofile(_file)
        _file.close()
        subprocess.call([Hadoop_bin, "fs", "-moveFromLocal", tmp_path+str(partition_id)+".edge", dst_path], stdout=subprocess.PIPE)
...
subprocess.call([Hadoop_bin, "fs", "-rmr", "/out_convert"], stdout=subprocess.PIPE)
subprocess.call(["rm "+tmp_path+"part-*"], shell=True, stdout=subprocess.PIPE)
sc.stop()
